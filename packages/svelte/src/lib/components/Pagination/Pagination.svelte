<script>
  ...
  /**
   * Sets the aria-label for the pagination component.
   * @type {string}
   */
   ...
   * @type {string}
   */
  export let previousLabel = 'Forrige side';

  /**
   * Sets the text label for the next page button.
   * @type {string}
   */
  export let nextLabel = 'Neste side';
  ...
  /**
   * Sets the size of the pagination-component.
   * @type {'sm' | 'md' | 'lg'}
   */
  export let size = 'md';
...
<style>
  .ds-pagination {
    ...
    :global(& > li:first-child > ::before, & > li:last-child > ::after) {
