<script>
  ...
  let isPartOfGroup = $derived(!!checkboxGroup);
  let groupUniqueId = $state(checkboxGroup?.uniqueId || uniqueId);
  let error = $derived(checkboxGroup?.error || '');
  ...
  let checkboxClasses = $derived(
    `ds-checkbox ds-checkbox--${standardizedSize} ${
      error ? 'ds-checkbox--error' : ''
    } ${readOnly || groupReadOnly ? 'ds-checkbox--readonly' : ''} ${class_}`,
  );
</script>
